...
# Характеристики, специфичные для набора "Программа лояльности"
LOYALTY_CHARACTERISTICS = [
    (
        "Тип программы",
        "Тип программы лояльности: кешбэк, баллы, мили, скидки и т.д.",
        "кешбэк / баллы / мили / скидки",
    ),
    (
        "Валюта бонусов",
        "Наименование бонусной единицы (рубли, баллы, бонусы, мили и т.п.)",
        "рубли / баллы / мили",
    ),
    (
        "Базовый процент начисления",
        "Процент или количество бонусов, начисляемых за каждые 1 BYN / 1 руб. трат в стандартных категориях",
        "% или бонусов за 1 BYN",
    ),
    (
        "Повышенный процент начисления",
        "Максимальный процент/количество бонусов в специальных или партнёрских категориях",
        "% или бонусов за 1 BYN",
    ),
    (
        "Категории повышенного начисления",
        "Категории трат, по которым действует повышенный процент начисления бонусов",
        "категории через запятую",
    ),
    (
        "Минимальная сумма для начисления",
        "Минимальная сумма одной транзакции, с которой начисляются бонусы",
        "в BYN",
    ),
    (
        "Лимит начисления бонусов",
        "Максимальное количество бонусов, которое можно накопить за месяц или год",
        "бонусов в месяц / год",
    ),
    (
        "Срок действия бонусов",
        "Срок, в течение которого бонусы сохраняются на счёте и не сгорают",
        "месяцев / лет / бессрочно",
    ),
    (
        "Условия списания бонусов",
        "На что можно потратить накопленные бонусы: покупки, скидка, перевод в деньги и т.д.",
        "оплата покупок / скидка / перевод",
    ),
    (
        "Минимальный порог списания",
        "Минимальное количество бонусов, необходимое для начала их списания",
        "бонусов",
    ),
    (
        "Курс конвертации бонусов",
        "Как бонусы соотносятся с реальными деньгами при списании (например: 1 балл = 0.01 BYN)",
        "1 бонус = X BYN",
    ),
    (
        "Партнёры программы",
        "Перечень ключевых партнёров, у которых доступны привилегии или повышенный кешбэк",
        "список партнёров",
    ),
    (
        "Мобильное приложение",
        "Наличие отдельного мобильного приложения программы лояльности для управления бонусами",
        "есть / нет / в банковском приложении",
    ),
    (
        "Реферальная программа",
        "Наличие реферальной системы: бонусы за приглашение друзей",
        "есть / нет",
    ),
    (
        "Стоимость участия",
        "Стоимость подключения или обслуживания программы лояльности (если применимо)",
        "в BYN в месяц / бесплатно",
    ),
]
...
def migrate_products():
    ...
    products_data = {
        ...
        "Программа лояльности": [
            ...
            ("Белгазпромбанк", "БУСЫ", "https://busy.by/"),
            ("Белагропромбанк", "бонусная программа", "https://belapb.bestcard.by/"),
            ("БелВэб", "ВUP4U", "https://www.belveb.by/online-services/up4u/"),
            ("Дабрабыт", "Программа лояльности", "https://bankdabrabyt.by/programma-loyalnosti/"),
        ]
    }
...
def migrate_banks():
    ...
    sets_data = [
        ...
        # ← новый набор
        ("Программа лояльности", "Программы лояльности банков", ",".join(all_banks)),
    ]
...
def migrate_loyalty_characteristics():
    """
    Миграция: добавляет характеристики из LOYALTY_CHARACTERISTICS
    в набор «Программа лояльности» для системного пользователя.
    Безопасна для повторного запуска — дубли не создаются.
    """
    db = SessionLocal()
    try:
        ...
        # Находим набор «Программа лояльности»
        loyalty_set = db.query(Set).filter(
            Set.name == "Программа лояльности",
            Set.user_id == sys_user.id
        ).first()

        if not loyalty_set:
            print("❌ Набор 'Программа лояльности' не найден. Сначала выполните migrate_banks().")
            return

        # Имена уже существующих характеристик в этом наборе
        existing_names = {
            c.name for c in db.query(Characteristic)
            .filter(Characteristic.set_id == loyalty_set.id).all()
        }

        added = 0
        for name, description, value_hint in LOYALTY_CHARACTERISTICS:
            if name not in existing_names:
                char = Characteristic(
                    user_id=sys_user.id,
                    set_id=loyalty_set.id,
                    name=name,
                    description=description,
                ...
                db.add(char)
                added += 1
                print(f"  ✅ Добавлена характеристика: {name}")
            else:
                print(f"  — Уже существует: {name}")

        db.commit()
        print(f"\n✅ migrate_loyalty_characteristics завершена. Добавлено: {added} характеристик.")
    except Exception as e:
